...
const getData = (endpoint, data, successCallback, errorCallback) => {
  ...
  const showErrorPopup = (message) => {
    ...
    const existingPopup = document.getElementById(errorPopupId);
    if (existingPopup) {
      existingPopup.remove();
    }

    const errorHTML = `
      <div id="${errorPopupId}" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: #e74c3c; color: white; padding: 15px; border-radius: 5px; z-index: 1001; display: flex; flexDirection: column; alignItems: center;">
        ${message}
      </div>
    `;
    ...
    setTimeout(() => {
      const popupToRemove = document.getElementById(errorPopupId);
      if (popupToRemove) {
